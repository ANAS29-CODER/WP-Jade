...
class OptionsPage extends Strategy {
	 ...
	 * @return bool
	 */
	private function isArrayOfStrings( $value ) {
		/**
		 * @param mixed      $val
		 ...
		 * @return bool
		 */
		$intIndexStringValue = function( $val, $index ) {
			return is_int( $index ) && is_string( $val );
		};

		return is_array( $value ) &&
			count( $value ) === wpml_collect( $value )->filter( $intIndexStringValue )->count();
	}
