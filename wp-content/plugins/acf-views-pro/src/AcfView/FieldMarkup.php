...
class FieldMarkup
{
    ...
    protected function getImageMarkup($fieldValue, bool $isWithSize = false,
                                      bool $isWithFullSizeUrlInData = false): string
    {
        $imageUrl = '';
    ...
    protected function getDatepickerMarkup(): string
    {
        ...
        $date = DateTime::createFromFormat($returnFormat, $this->fieldValue);
    ...
    protected function getPageLinkMarkup(): string
    {
        ...
        foreach ($links as $linkUrl) {
            ...
            $markup .= "<a target='_self' class='acf-view__link' href='" . esc_attr($linkUrl) . "'>" .
                esc_html($linkTitle) . "</a>";
        }
    ...
    protected function getTaxonomyMarkup(): string
    {
        ...
        foreach ($this->fieldValue as $term) {
    ...
    public function getMarkup(int $viewId): string
    {
        ...
        if (!$this->fieldMeta->isFieldExist() ||
            !in_array($this->fieldMeta->getType(), $this->acf->getFieldTypes(), true)) {
            return '';
        }
