...
class Acf
{
    ...
    protected function getGroupChoices(bool $isWithExtra = true): array
    {
        ...
        if ($isWithExtra) {
            $groupChoices[self::GROUP_POST] = '$Post$';
            $groupChoices[self::GROUP_TAXONOMY] = '$Taxonomy$';
        }
    ...
    protected function getFieldChoices(bool $isWithExtra = true, array $excludeTypes = []): array
    {
        ...
        if ($isWithExtra) {
            $fieldChoices = array_merge($fieldChoices, [
                Field::createKey(self::GROUP_POST, Post::FIELD_TITLE) => 'Title',
                Field::createKey(self::GROUP_POST, Post::FIELD_TITLE_LINK) => 'Title with link',
                Field::createKey(self::GROUP_POST, Post::FIELD_EXCERPT) => 'Excerpt',
                Field::createKey(self::GROUP_POST, Post::FIELD_THUMBNAIL) => 'Thumbnail',
                Field::createKey(self::GROUP_POST, Post::FIELD_AUTHOR) => 'Author',
                Field::createKey(self::GROUP_POST, Post::FIELD_DATE) => 'Published date',
                Field::createKey(self::GROUP_POST, Post::FIELD_MODIFIED) => 'Modified date',
                Field::createKey(self::GROUP_POST, Post::FIELD_LINK) => 'Link',
            ]);
    ...
    protected function getRepeaterChoices(array $excludeTypes = []): array
    {
        $repeaterChoices = [
            '' => 'Select',
        ];
        ...
        foreach ($groups as $group) {
            ...
            foreach ($fields as $groupField) {
                ...
                if ('repeater' !== $groupField['type'] ||
                    !$subFields) {
                    continue;
                }

                foreach ($subFields as $subField) {
                    // repeater in repeater is not allowed
                    if (!in_array($subField['type'], $supportedFieldTypes, true) ||
                        'repeater' === $subField['type'] ||
                    ...
                    $repeaterChoices[$fullFieldId] = $subField['label'] . ' (' . $subField['type'] . ')';
                }
            }
        }

        return $repeaterChoices;
    }
    ...
    protected function setFieldSelectChoices(): void
    {
        ...
        add_filter(
            ...
            function (array $field) {
                $field['choices'] = $this->getRepeaterChoices();
    ...
    protected function addConditionalFilter(string $fieldName, array $notFieldTypes, bool $isRepeater = false,
                                            array  $includeFields = []): void
    {
        $acfFieldName = !$isRepeater ?
        ...
        add_filter(
            ...
            function (array $field) use ($acfKey, $notFieldTypes, $includeFields, $isRepeater) {
                // using exactly the negative (excludeTypes) filter,
                // otherwise if there are no such fields the field will be visible
                $notRightFields = !$isRepeater ?
                    $this->getFieldChoices(true, $notFieldTypes) :
                    $this->getRepeaterChoices($notFieldTypes);
    ...
    protected function setViewConditionalRules(): void
    {
        ...
        $masonryFields = [Field::FIELD_MASONRY_ROW_MIN_HEIGHT,
            Field::FIELD_MASONRY_GUTTER, Field::FIELD_MASONRY_MOBILE_GUTTER,];
        ...
        $masonryRepeaterFields = [RepeaterField::FIELD_MASONRY_ROW_MIN_HEIGHT,
            RepeaterField::FIELD_MASONRY_GUTTER, RepeaterField::FIELD_MASONRY_MOBILE_GUTTER,];
        ...
        add_filter(
            ...
            function (array $field) {
                // using exactly the negative (excludeTypes) filter,
                // otherwise if there are no such fields the field will be visible
                $notRepeaterFields = $this->getFieldChoices(true, ['repeater',]);
