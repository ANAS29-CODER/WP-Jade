...
class Plugin
{
    ...
    protected string $name = 'ACF Views';
    ...
    protected static function getErrorMarkup(string $shortcode, array $args, string $error): string
    {
        ...
        return sprintf(
            "<p style='color:red;'>Shortcode error: %s %s</p>",
            $error,
    ...
    public function showWarningAboutInactiveAcfPlugin(): void
    {
        ...
        echo sprintf(
            ...
            '<p>"ACF Views" plugin requires <a target="_blank" href="%s">Advanced Custom Fields</a> plugin to be enabled on your website 
(either <a target="_blank" href="%s">Free</a> or <a target="_blank" href="%s">Pro</a> version).' .
            ' Please enable "Advanced Custom Fields" plugin or "ACF Views" plugin won\'t work properly.</p>' .
            '</div>',
    ...
    public function showWarningAboutOpcacheIssue(): void
    {
        ...
        $readMoreLink = sprintf('<a target="_blank" href="%s">Read more</a>', self::CONFLICTS_URL);
        printf('<div class="notice notice-error"><p>Compatibility issue detected! "ACF Views" plugin requires "PHPDoc" comments in code. 
<br>Please change the "opcache.save_comments" option in your php.ini file to the default value of "1" on your hosting. %s
</p></div>', $readMoreLink);
    }
    ...
    public function acfCardsShortcode($attrs): string
    {
        ...
        if (!$acfCardPost ||
            ...
            return self::getErrorMarkup(self::SHORTCODE_CARDS, $attrs, 'card-id attribute is missing or wrong');
        }
    ...
    public function acfViewsShortcode($attrs): string
    {
        ...
        if (!$acfViewPost ||
            ...
            return self::getErrorMarkup(self::SHORTCODE, $attrs, 'view-id attribute is missing or wrong');
        }
        ...
        if (!$dataPostId) {
            return self::getErrorMarkup(self::SHORTCODE, $attrs, 'object-id argument contains the wrong value');
        }
    ...
    public function showPluginDeactivatedNotice(): void
    {
        ...
        $message = sprintf(
            '"ACF Views" and "ACF Views Pro" should not be active at the same time. We\'ve automatically deactivated "%s".',
    ...
    public function amendProFieldLabelAndInstruction(array $field): array
    {
        ...
        if ($isProField) {
            if ('tab' === $type) {
                ...
                $field['instructions'] = sprintf(
                    '<a href="%s" target="_blank">Upgrade to Pro</a> to unlock. %s',
        ...
        if ($isAcfProField) {
            $field['instructions'] = sprintf(
                '(<a href="%s" target="_blank">ACF Pro</a> version is required for this feature) %s',
    ...
    public function printSurveyLink(string $html): string
    {
        ...
        $content = sprintf(
            'Thank you for creating with <a target="_blank" href="%s">WordPress</a> and <a target="_blank" href="%s">ACF Views</a>.',
            'https://wordpress.org/',
            self::BASIC_VERSION_URL
        );
        $content .= " " . sprintf(
                "<span>Take <a target='_blank' href='%s'>2 minute survey</a> to improve the ACF Views plugin.</span>",
                self::SURVEY_URL
            );
